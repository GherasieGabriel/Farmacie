...
public class PanelAngajat extends JPanel {
    ...
    public PanelAngajat() {
        ...
        model.setColumnIdentifiers(new String[]{"ID", "Nume", "Prenume", "Post", "Salariu", "Farmacie"});
        table = new JTable(model);
        add(new JScrollPane(table), BorderLayout.CENTER);

        JPanel panelButoane = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 5));
        panelButoane.add(createButton("Adauga", e -> { new DialogAngajat(null).setVisible(true); reincarca(); }));
    ...
    private void reincarca() {
        ...
        try {
            ...
            ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM Angajat ORDER BY Nume");
            while (rs.next()) {
                model.addRow(new Object[]{
                        rs.getInt(1), rs.getString(2), rs.getString(3),
                        rs.getString(4), rs.getBigDecimal(5), rs.getInt(6)
                });
            }
    ...
    private void modifica() {
        ...
        new DialogAngajat(null, (Integer) model.getValueAt(row, 0)).setVisible(true);
        reincarca();
    }
    ...
    private void sterge() {
        ...
        if (JOptionPane.showConfirmDialog(this, "Confirmati stergerea?") == JOptionPane.YES_OPTION) {
            try {
                Connection conn = DatabaseConnection.getConnection();
                PreparedStatement ps = conn.prepareStatement("DELETE FROM Angajat WHERE ID_Angajat = ?");
